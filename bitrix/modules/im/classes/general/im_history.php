...
class CIMHistory
{
		...
		if ($chatId > 0)
		{
			...
			while ($arRes = $orm->fetch())
			{
				...
				$arMessages[$arRes['ID']] = Array(
					...
					'date' => $arRes['DATE_CREATE']->getTimestamp(),
	...
	function SearchDateMessage($searchDate, $toUserId, $fromUserId = false, $bTimeZone = true)
	{
		...
		if ($chatId > 0)
		{
			...
			while ($arRes = $dbRes->Fetch())
			{
				...
				$arMessages[$arRes['ID']] = Array('id' => $arRes['ID'], 'chatId' => $arRes['CHAT_ID'], 'senderId' => $arRes['FROM_USER_ID'], 'recipientId' => $arRes['TO_USER_ID'], 'date' => $arRes['DATE_CREATE'], 'system' => $arRes['NOTIFY_EVENT'] == 'private' ? 'N' : 'Y', 'text' => \Bitrix\Im\Text::parse($arRes['MESSAGE']));
	...
	public function GetMessagesByDate($chatId, $date, $messageStart = 0, $timezone = true)
	{
		...
		return Array(
			...
			'woUserInGroup' => $users['woUserInGroup'],
			'phones' => $users['phones'],
