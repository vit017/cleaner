	...
	public static function getOrderPropertiesByJS(Order $order)
	{
		...
		if (!empty($result['groups']) && !empty($result['properties']))
		{
			...
			if (!empty($groupIndexList))
			{
				...
				{
					if($propertyData['ID']==9){
                        $r=\CIBlockElement::GetPropertyValues(10, array('ACTIVE' => 'Y'),false,array('ID'=>array(61,62)));
                        while($rr = $r->GetNext(false,false)) {
                            $datesArr=explode(',',$rr['62']);
                            if(in_array($workDate,$datesArr)){
                                unset($result['properties'][$index]['OPTIONS'][$rr['61']]);
                            }
                        }
					}
