...
$arAllOptions = array(
	...
	GetMessage("altasib_geoip_options_geoiplibru"),
	Array("note" => GetMessage("ALTASIB_GEOIP_NEW_RULES")),
	),
);
//separate settings for each site_code
$siteList = Bitrix\Main\SiteTable::GetList(Array(
	'filter' => Array("ACTIVE" => "Y")
));
while($site = $siteList-> Fetch())
{
	$arAllOptions["main"][] = Array($site["LID"] . "_site_code", GetMessage("ALTASIB_GEOIP_CODE_SITES", Array("#SITE#" => "<b title='".$site["SITE_NAME"].($site["SERVER_NAME"] ? " [".$site["SERVER_NAME"]."]" : "")."'>(".$site["LID"].") ".$site["NAME"]."</b>")), "", Array("text"));
}
