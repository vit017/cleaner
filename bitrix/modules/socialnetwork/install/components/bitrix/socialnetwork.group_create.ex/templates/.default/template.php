...
// this situation is impossible now but this code may be needed in the future
		...
		<form method="post" name="sonet_group_create_popup_form" id="sonet_group_create_popup_form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data"><?
			...
			?><div id="sonet_group_create_popup" class="sonet-group-create-popup"><?
				...
				if (empty($arResult["TAB"]) || $arResult["TAB"] == "edit")
				{
					?><div class="popup-window-tabs"><?
						?><span class="popup-window-tab<?=($active_tab == "additional" ? "" : " popup-window-tab-selected")?>"><?=GetMessage("SONET_GCE_TAB_1")?></span><?
						?><span class="popup-window-tab"><?=GetMessage("SONET_GCE_TAB_2")?></span><?

						if (!array_key_exists("TAB", $arResult))
						{
							?><span class="popup-window-tab"><?=GetMessage("SONET_GCE_TAB_3")?></span><?
						}

						?><span class="popup-window-tab<?=($active_tab == "additional" ? " popup-window-tab-selected" : "")?>"><?=GetMessage("SONET_GCE_TAB_4")?></span><?
					?></div><?
				}

				?><div id="sonet_group_create_tabs_content" class="popup-window-tabs-content<?=($arResult["bExtranetInstalled"] ? " sonet-group-create-tabs-content-extranet" : "")?><?=($arResult["TAB"] == "invite" ? " sonet-group-create-tabs-content-invite" : "")?>"><?

					if (!array_key_exists("TAB", $arResult) || $arResult["TAB"] == "edit")
					{
						?><div class="popup-window-tab-content<?=($active_tab == "additional" ? "" : " popup-window-tab-content-selected")?>" id="sonet_group_create_tabs_description"><?
					...
					{
						?><div class="popup-window-tab-content" id="sonet_group_create_tabs_features">
					...
					if (!array_key_exists("TAB", $arResult) || $arResult["TAB"] == "invite")
					{
						...
						?><div class="popup-window-tab-content<?=($arResult["TAB"] != "invite" ? '' : ' popup-window-tab-content-selected')?>" id="sonet_group_create_tabs_invite"><?
					...
					if (!array_key_exists("TAB", $arResult) || $arResult["TAB"] == "edit")
					{
						?><div class="popup-window-tab-content<?=($active_tab == "additional" ? ' popup-window-tab-content-selected' : '')?>" id="sonet_group_create_tabs_additional">
