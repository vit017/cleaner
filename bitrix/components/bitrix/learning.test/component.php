...
$arParams["GRADEBOOK_TEMPLATE"] = (strlen($arParams["GRADEBOOK_TEMPLATE"]) > 0 ? htmlspecialchars($arParams["GRADEBOOK_TEMPLATE"]) : "");
...
$bCheckPerm = ($APPLICATION->GetUserRight("learning") < "W" && !$USER->IsAdmin());
...
{
	$rsPrevTest = CTest::GetByID($arTest["PREVIOUS_TEST_ID"]);
	if ($arPrevTest = $rsPrevTest->GetNext())
	{
		if ($parent = $this->GetParent())
		{
			...
			$errors[] = str_replace("#TEST_LINK#", "\"<a href=\"".$testUrlTemplate."\">".$arPrevTest["NAME"]."</a>\"", GetMessage("LEARNING_TEST_DENIED_PREVIOUS"));
		}
	}
}
...
if (!sizeof($errors)) {
	...
	if ($bTestCreate)
	{
		...
		if ($arAttempt = _AttemptExists($arParams["TEST_ID"]))
		{
			...
			{
				$arFields = array(
					"DATE_START" => date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL")), time()),
					"DATE_END" => false,
					"STATUS" => "B"
				);
	...
	elseif ($bPostAnswer)
	{
		...
		{
			$result = CTestResult::AddResponse(intval($_REQUEST["TEST_RESULT"]), $_REQUEST["answer"]);
			if(!$result)
			{
				$sessAttemptID = null;
			...
			elseif ($arTest["SHOW_ERRORS"] == "Y" && $result["CORRECT"] == "N" && $result["ANSWERED"] == "Y")
			{
	...
	$arResult["SAFE_REDIRECT_PAGE"] = htmlspecialchars($arResult["REDIRECT_PAGE"]);
}
