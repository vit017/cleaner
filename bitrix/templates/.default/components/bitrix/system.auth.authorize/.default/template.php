...
<div class="auth-block">
    <form class="auth-form js-form-validate" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="/auth/auth_phone.php">
        ...
        <? if ($_GET['ERROR']){?>
            <span class="input-txt__error">Не верный логин или пароль</span>
        <?}?>
        <label class="input-txt input-txt_width_full <? if ($_GET['ERROR'])  {?>input-txt_state_error<?}?>" data-placeholder="Пароль">
