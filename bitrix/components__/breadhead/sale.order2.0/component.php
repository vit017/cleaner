...
if ( true)
{
	...
	if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 6)
	{
		...
		if ( $totalTime > 16-bhSettings::$SaveConst ){
			$arResult["ERROR_MESSAGE"] = "На уборку Вашей квартиры требуется более ".(16-bhSettings::$SaveConst)." часов! Напишите, пожалуйста, нам через <a href='/help/?#form'>форму обратной связи</a> и мы обязательно свяжемся с Вами для обсуждения индивидуальных условий";
			$bProductsInBasket = false;
		}
		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 2)
		{
			...
			if ( $arResult["CurrentStep"] > 2 ){
				...
				if ( $arResult['POST']['RESEND'] == 'on' ){
					...
					if ( bhTools::checkConfirm($phone, $arResult['POST']['confirm_code']) ){
						bhTools::setConfirm($phone, true, $arResult['POST']['confirm_code']);
						$arResult["CurrentStep"] = 3;
					}else{
